...
from plot_template import (
    ...
    make_plot_frequency_script,
    make_plot_ships_script,
)
...
max_wait_durations = [
    0.0 + i * 120.0 for i in range(5)
]  # all these values will be attempted
    ...
    for strategy in strategies:
        ...
        with open(filename, "w") as f:
            try:
                ...
                raise Exception(
                    "There was an IndexError, meaning that fewer ships have made it to the sink than expected.\nYour model is not (yet) correct."
                ) from e

        # Generate gnuplot code:
        for f, col in [
            (make_plot_ships_script, plots_ships),
